...
if [[ "$latest_version" == "$current_version" ]]; then
  echo "gastown is already up-to-date at version $latest_version"
  exit 0
fi

echo "Updating gastown from $current_version to $latest_version"

sed -i -E "s/^( *version = \").*(\";)/\1$latest_version\2/" "$DEFAULT_NIX_FILE"

hash_base64=$(nix-prefetch-url --unpack --type sha256 \
  "https://github.com/steveyegge/gastown/archive/refs/tags/v${latest_version}.tar.gz" 2>/dev/null)
...
vendor_hash=$(nix build "${NUR_ROOT}#gastown" 2>&1 | grep "got:" | awk '{print $NF}' || true)
...
echo "Successfully updated gastown to version $latest_version"
