...
echo "Fetching latest release information for steveyegge/beads..."
latest_version=$(curl --silent --fail "https://api.github.com/repos/steveyegge/beads/releases/latest" | jq -r .tag_name | sed 's/^v//')
current_version=$(grep 'version = "' "$DEFAULT_NIX_FILE" | head -n1 | cut -d '"' -f 2)

if [[ "$latest_version" == "$current_version" ]]; then
  echo "beads is already up-to-date at version $latest_version"
  exit 0
fi

echo "Updating beads from $current_version to $latest_version"

# Update version
sed -i -E "s/^( *version = \").*(\";)/\1$latest_version\2/" "$DEFAULT_NIX_FILE"

# Update source hash
url="https://github.com/steveyegge/beads/archive/refs/tags/v${latest_version}.tar.gz"
...
vendor_hash=$(nix build "${NUR_ROOT}#beads" 2>&1 | grep "got:" | awk '{print $NF}' || true)
...
echo "Successfully updated beads to version $latest_version"
