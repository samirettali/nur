...
latest_version=$(curl --silent --fail ${GH_TOKEN:+-H "Authorization: Bearer $GH_TOKEN"} "https://api.github.com/repos/AkashRajpurohit/git-sync/releases/latest" | jq -r .tag_name | sed 's/^v//')
