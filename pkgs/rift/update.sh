...
echo "Fetching latest release information for acsandmann/rift..."
latest_version=$(curl --silent --fail ${GH_TOKEN:+-H "Authorization: Bearer $GH_TOKEN"} "https://api.github.com/repos/acsandmann/rift/releases/latest" | jq -r .tag_name | sed 's/^v//')
current_version=$(grep 'version = "' "$DEFAULT_NIX_FILE" | head -n1 | cut -d '"' -f 2)

if [[ "$latest_version" == "$current_version" ]]; then
	echo "rift is already up-to-date at version $latest_version"
	exit 0
fi

echo "Updating rift from $current_version to $latest_version"

# Update version
sed -i -E "s/^( *version = \").*(\";)/\1$latest_version\2/" "$DEFAULT_NIX_FILE"

# Update source hash
url="https://github.com/acsandmann/rift/archive/refs/tags/v${latest_version}.tar.gz"
...
cargo_hash=$(nix build --impure --expr "let repo = import ${NUR_ROOT} {}; in repo.rift.cargoDeps" 2>&1 | awk '/got:/ { print $NF }' | tail -n1 || true)
...
echo "Successfully updated rift to version $latest_version"
