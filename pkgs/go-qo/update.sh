...
latest_version=$(curl --silent --fail ${GH_TOKEN:+-H "Authorization: Bearer $GH_TOKEN"} "https://api.github.com/repos/kiki-ki/go-qo/releases/latest" | jq -r .tag_name | sed 's/^v//')
...
vendor_hash=$(nix build --impure --expr "let repo = import ${NUR_ROOT} {}; in repo.go-qo.goModules" 2>&1 | awk '/got:/ { print $NF }' | tail -n1 || true)
