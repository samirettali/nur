...
if [[ "$latest_version" == "$current_version" ]]; then
  echo "sottomano is already up-to-date at version $latest_version"
  exit 0
fi

echo "Updating sottomano from $current_version to $latest_version"
sed -i -E "s/^( *version = \").*(\";)/\1$latest_version\2/" "$DEFAULT_NIX_FILE"

url="https://github.com/samirettali/sottomano/releases/download/v${latest_version}/Sottomano-${latest_version}.dmg"
...
echo "Successfully updated sottomano to version $latest_version"
