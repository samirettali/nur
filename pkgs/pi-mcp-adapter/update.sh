...
latest_version=$(curl --silent --fail \
  ${GH_TOKEN:+-H "Authorization: Bearer $GH_TOKEN"} "https://api.github.com/repos/nicobailon/pi-mcp-adapter/releases/latest" \
  | jq -r .tag_name | sed 's/^v//')
current_version=$(grep 'version = "' "$DEFAULT_NIX_FILE" | head -n1 | cut -d '"' -f 2)

if [[ "$latest_version" == "$current_version" ]]; then
  echo "pi-mcp-adapter is already up-to-date at version $latest_version"
  exit 0
fi

echo "Updating pi-mcp-adapter from $current_version to $latest_version"

sed -i -E 's/^( *version = ").*(";)/\1'"$latest_version"'\2/' "$DEFAULT_NIX_FILE"

url="https://github.com/nicobailon/pi-mcp-adapter/archive/refs/tags/v${latest_version}.tar.gz"
...
with urllib.request.urlopen(
    f"https://raw.githubusercontent.com/nicobailon/pi-mcp-adapter/v{version}/package-lock.json"
) as response:
...
nix build --no-link --impure --expr \
  "let repo = import ${NUR_ROOT} {}; in repo.pi-mcp-adapter"

echo "Successfully updated pi-mcp-adapter to version $latest_version"
